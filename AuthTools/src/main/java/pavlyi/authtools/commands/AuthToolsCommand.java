...
import pavlyi.authtools.handlers.ImportHandler;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("authtools")) {
			if (!(sender instanceof Player)) {
				...
				if (args.length == 1) {
					if (!(args[0].equalsIgnoreCase("reset") || args[0].equalsIgnoreCase("backend")
							|| args[0].equalsIgnoreCase("info") || args[0].equalsIgnoreCase("about")
							|| args[0].equalsIgnoreCase("reload") || args[0].equalsIgnoreCase("setspawn")
							|| args[0].equalsIgnoreCase("setlobby") || args[0].equalsIgnoreCase("import"))) {
						for (String tempMessage : instance.getMessagesHandler().COMMANDS_AUTHTOOLS_HELPUSAGE) {
							sender.sendMessage(tempMessage);
						}
					} else {
					...
					if (!(args[0].equalsIgnoreCase("reset") || args[0].equalsIgnoreCase("backend")
						...
						if (args[0].equalsIgnoreCase("backend")) {
							...
							if (enteredConnectionType.equalsIgnoreCase("yaml")
									|| enteredConnectionType.equalsIgnoreCase("mysql")
									|| enteredConnectionType.equalsIgnoreCase("sqlite")
									|| enteredConnectionType.equalsIgnoreCase("mongodb")) {
								if (enteredConnectionType.equalsIgnoreCase("yaml")) {
								...
								if (enteredConnectionType.equalsIgnoreCase("mongodb")) {
									if (connectionType.equals("MONGODB")) {
										sender.sendMessage(instance.getMessagesHandler().COMMANDS_AUTHTOOLS_BACKEND_ALREADY_CONNECTED
														.replace("%connectionType%",
																enteredConnectionType.toUpperCase()));
										return true;
									}
									...
									instance.switchConnection(enteredConnectionType);
									sender.sendMessage(instance.getMessagesHandler().COMMANDS_AUTHTOOLS_BACKEND_SWITCHED_TO_CONNECTION
													.replace("%connectionType%", enteredConnectionType.toUpperCase()));
								}
							...
							sender.sendMessage(instance.getMessagesHandler().COMMANDS_AUTHTOOLS_BACKEND_WRONG_CONNECTION_TYPE
											.replace("%connectionType%", enteredConnectionType.toUpperCase()));
						}
						...
						if (args[0].equalsIgnoreCase("info")) {
							...
							if (user.isInDatabase()) {
								...
								sender.sendMessage(
										instance.getMessagesHandler().PLAYER_NOT_FOUND.replace("%player%", args[1]));
							}
						}
						...
						if (args[0].equalsIgnoreCase("import")) {
							...
							if (type.equalsIgnoreCase("yaml") || type.equalsIgnoreCase("mysql") || type.equalsIgnoreCase("mongodb") || type.equalsIgnoreCase("sqlite"))
								validType = true;

							if (!validType) {
								sender.sendMessage(instance.getMessagesHandler().COMMANDS_AUTHTOOLS_IMPORT_INCORRECT_TYPE);
							} else {
								ImportHandler importHandler = new ImportHandler(type);

								if (importHandler.importYAML() || importHandler.importMySQL() || importHandler.importMongoDB() || importHandler.importSQLite()) {
									sender.sendMessage(instance.getMessagesHandler().COMMANDS_AUTHTOOLS_IMPORT_SUCESSFULLY_IMPORTED);
								} else {
						...
						if (args[0].equalsIgnoreCase("reset")) {
							...
							if (type.equalsIgnoreCase("2fa") || type.equalsIgnoreCase("mail")) {
								...
								if (type.equalsIgnoreCase("mail")) {
									sender.sendMessage(instance.color(
											"&f[&cAuthTools&f] &cEmail &fis in progress! Keep watching for an &cupdate&f!"));
								}
				...
				if (!p.hasPermission("authtools.use") || !p.hasPermission("authtools.*")) {
					...
					if (args.length == 0) {
						instance.getMessagesHandler().sendMessage(p,
								instance.getMessagesHandler().COMMANDS_AUTHTOOLS_HELPUSAGE);
					}
					...
					if (args.length == 1) {
						if (!(args[0].equalsIgnoreCase("reset") || args[0].equalsIgnoreCase("backend")
							...
							if (args[0].equalsIgnoreCase("import")) {
								if (p.hasPermission("authtools.use.import") || p.hasPermission("authtools.*")) {
									p.sendMessage(instance.getMessagesHandler().COMMANDS_AUTHTOOLS_IMPORTUSAGE);
								} else {
									p.sendMessage(instance.getMessagesHandler().NO_PERMISSIONS);
								}
							}
						...
						if (!(args[0].equalsIgnoreCase("reset") || args[0].equalsIgnoreCase("backend")
							...
							if (args[0].equalsIgnoreCase("backend")) {
								if (p.hasPermission("authtools.use.backend") || p.hasPermission("authtools.*")) {
									...
									p.sendMessage(instance
											.getMessagesHandler().COMMANDS_AUTHTOOLS_BACKEND_WRONG_CONNECTION_TYPE
													.replace("%connectionType%", enteredConnectionType.toUpperCase()));
								} else {
									p.sendMessage(instance.getMessagesHandler().NO_PERMISSIONS);
								}
							...
							if (args[0].equalsIgnoreCase("info")) {
								if (p.hasPermission("authtools.use.info") || p.hasPermission("authtools.*")) {
									...
									if (user.isInDatabase()) {
										...
										for (String tempMessage : instance
												.getMessagesHandler().COMMANDS_AUTHTOOLS_INFO) {
											tempMessage = tempMessage.replace("%player%", args[1]);
										...
										p.sendMessage(instance.getMessagesHandler().PLAYER_NOT_FOUND.replace("%player%",
												args[1]));
									}
								} else {
									p.sendMessage(instance.getMessagesHandler().NO_PERMISSIONS);
							...
							if (args[0].equalsIgnoreCase("import")) {
								if (p.hasPermission("authtools.use.import") || p.hasPermission("authtools.*")) {
									if (args[0].equalsIgnoreCase("import")) {
										...
										if (!validType) {
											p.sendMessage(instance.getMessagesHandler().COMMANDS_AUTHTOOLS_IMPORT_INCORRECT_TYPE);
										} else {
											ImportHandler importHandler = new ImportHandler(type);

											if (importHandler.importYAML() || importHandler.importMySQL() || importHandler.importMongoDB() || importHandler.importSQLite()) {
												p.sendMessage(instance.getMessagesHandler().COMMANDS_AUTHTOOLS_IMPORT_SUCESSFULLY_IMPORTED);
											} else {
												p.sendMessage(instance.getMessagesHandler().COMMANDS_AUTHTOOLS_IMPORT_ERROR_WHILE_IMPORTING);
											}
										}
									}
								} else {
									...
									if (type.equalsIgnoreCase("2fa") || type.equalsIgnoreCase("mail")) {
										if (type.equalsIgnoreCase("2fa")) {
											if (user.get2FA()) {
												...
												p.sendMessage(instance
														.getMessagesHandler().COMMANDS_AUTHTOOLS_RESET_DISABLED_2FA
