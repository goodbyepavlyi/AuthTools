...
public class AuthHandler {
    ...
    public void requestRegister() {
        ...
        if (instance.getConfigHandler().SETTINGS_RESTRICTIONS_TELEPORT_UNAUTHED_TO_SPAWN)
            if (Variables.getSpawn("authenticationSpawn") != null)
                player.teleport(Variables.getSpawn("authenticationSpawn"));
        ...
        if (!Variables.getVersion().equals(VersionType.ONE_NINE))
            user.setPlayerInventory(player.getInventory());
    ...
    public void requestLogin() {
        ...
        String[] loginMessageArgs = instance.getMessagesHandler().COMMANDS_2FA_LOGIN_LOGIN_MESSAGE.split("\\\\n");
        for (String loginMessage : loginMessageArgs)
        ...
        try {
            if (user.get2FAsecret() != null) {
                if (new GoogleAuthenticator().authorize(user.get2FAsecret(), Integer.parseInt(code))) {
                    ...
                    if (Variables.getSpawn("lobby") != null)
                        player.teleport(Variables.getSpawn("lobby"));
        ...
        try {
            if (user.get2FAsecret() != null) {
                if (new GoogleAuthenticator().authorize(user.get2FAsecret(), Integer.parseInt(code))) {
                    ...
                    user.needsToBeAuthenticated(false);
                    if (instance.getConfigHandler().SETTINGS_SESSION_ENABLE)
                        user.setSession((System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(instance.getConfigHandler().SETTINGS_SESSION_TIMEOUT)));
    ...
    private void forceLogin() {
        ...
        instance.getPluginManager().callEvent(new AsyncAuthenticateEvent(player, AuthenticationResult.SESSION, ""));
    }
